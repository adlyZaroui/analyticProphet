...
class CustomProphet:
    ...
    def _minus_log_posterior(self, params: np.array) -> float:
        ...
        minus_log_posterior = np.sum((y_pred-y_true)**2) / (2*self.sigma_obs**2) + k**2 / 2*self.sigma_k**2  + m**2 / 2*self.sigma_m**2 + np.sum(beta**2) / (2*self.sigma**2) + np.sum(np.abs(delta)) / self.tau        
    ...
    def _gradient(self, params: np.array) -> np.array:
        ...
        dk = np.array([- 2 * np.dot(r,self.t_scaled) + k / self.sigma_k**2])
        dm = np.array([- 2 * np.sum(r) + m / self.sigma_m**2])
        ddelta = - 2 * np.sum(r[:, None] * A * self.t_scaled[:, None], axis=0) + np.sign(delta) / self.tau
        dbeta = - 2 * np.dot(r, x) + beta / self.sigma**2
    ...
    def _minus_log_posteriorAndGradient(self, params: np.array) -> Tuple[float, np.array]:
        ...
        minus_log_posterior = np.sum(r**2) / (2*self.sigma_obs**2) + k**2 / 2*self.sigma_k**2  + m**2 / 2*self.sigma_m**2 + np.sum(beta**2) / (2*self.sigma**2) + np.sum(np.abs(delta)) / self.tau
    ...
    def fit(self, df: pd.DataFrame, analytic: bool=False, use_combined: bool=False) -> Tuple[float, float, np.array, np.array]:
    ...
    def predicte(self, future_df):
        ...
        gamma = -self.change_points * delta
        g = (k + det_dot(A, delta)) * future_df['t_scaled'].values + (m + det_dot(A, gamma))
        ...
        forecast['yhat_lower'] = quantiles[0, :]
        forecast['yhat_upper'] = quantiles[1, :]
    ...
    def predicte(self, future_df):
        ...
        forecast['yhat_lower'] = forecast['trend_lower'] + seasonality
        forecast['yhat_lower'] = forecast['yhat_lower'] * self.y_absmax
        
        forecast['yhat_upper'] = forecast['trend_upper'] + seasonality
        forecast['yhat_upper'] = forecast['yhat_upper'] * self.y_absmax
