...
class CustomProphet:
    ...
    def _minus_log_posterior(self, params: np.array) -> float:
        ...
        #A = (self.t_scaled[:, None] > np.array(self.change_points)) * 1
        gamma = -self.change_points * delta
        #g = (k + np.dot(A, delta)) * self.t_scaled + (m + np.dot(A, gamma))
        g = (k + det_dot(A, delta)) * self.t_scaled + (m + det_dot(A, gamma))
    ...
    def _gradient(self, params: np.array) -> np.array:
        ...
        ddelta = -np.sum(r[:, None] * self.t_scaled[:, None] * A, axis=0) / self.sigma_obs**2 + np.sign(delta) / self.tau
        #ddelta = -np.sum(r * self.t_scaled * A[:,], axis=0) / self.sigma_obs**2 + np.sign(delta) / self.tau
        #ddelta = -np.sum([r[i] * self.t_scaled[i] * A[i, :] for i in range(len(r))], axis=0) / self.sigma_obs**2 + np.sign(delta) / self.tau
        #ddelta = -np.sum([r[i] * A[i, :] * self.t_scaled[i] for i in range(len(r))], axis=0) / self.sigma_obs**2 + np.sum(np.sign(delta)) / self.tau
        #ddelta = -np.sum(r[:, None] * A * self.t_scaled[:, None], axis=0) / self.sigma_obs**2 + np.sign(delta) / self.tau
        #ddelta = -np.sum(r * A * self.t_scaled[:, None], axis=0) / self.sigma_obs**2 + np.sign(delta) / self.tau
        #ddelta = -np.sum((r[:, None] * self.t_scaled[:, None]) * A, axis=1) / self.sigma_obs**2 + np.sign(delta) / self.tau
        #ddelta = -np.sum(r[:, None] * self.t_scaled[:, None] * A, axis=0) / self.sigma_obs**2 + np.sign(delta) / self.tau
        #ddelta  = -np.sum(r * self.t_scaled * A) / self.sigma_obs**2 + np.sign(delta) / self.tau
        #ddelta  = -np.sum(r[:, None] * self.t_scaled[:, None] * A) / self.sigma_obs**2 + np.sign(delta) / self.tau
        #ddelta  = -np.sum(r[:, None] * self.t_scaled[:, None] * A) / self.sigma_obs**2 + np.sign(delta) / self.tau
    ...
    def check_gradients(self, params):
        eps = 1e-5
        num_grad = np.zeros_like(params)
        for i in range(len(params)):
            params[i] += eps
            f_plus = self._minus_log_posterior(params)
            params[i] -= 2 * eps
            f_minus = self._minus_log_posterior(params)
            num_grad[i] = (f_plus - f_minus) / (2 * eps)
            params[i] += eps
        return np.allclose(num_grad, self._gradient(params), atol=1)
        
    def fite(self, df: pd.DataFrame, analytic: bool=False, use_combined: bool=False) -> Tuple[float, float, np.array, np.array]:
    ...
    def fit(self, df: pd.DataFrame, analytic: bool=False, use_combined: bool=False, optimizer: str='L-BFGS-B') -> Tuple[float, float, np.array, np.array]:
        ...
        if use_combined:
            #assert self.check_gradients(initial_params_array), "Gradients do not match"
            opt_params = minimize(self._minus_log_posteriorAndGradient,
                    initial_params_array,
                    method=optimizer,
                    options={'maxiter': 10000},
